...
while ($fila = sqlsrv_fetch_array($consultaEjecucion, SQLSRV_FETCH_ASSOC)) {
    ...
    $areaHil = $fila['cantidadAreaTotal'];
    $campo = $fila['campo'];
    $cc = $fila['centroCosto'];
...
# lista maquinarias
$consulta11 = sqlsrv_query($conn,("SELECT * FROM Campo WHERE nombre = '$campo'" ));
while ($fila = sqlsrv_fetch_array($consulta11, SQLSRV_FETCH_ASSOC)) {
    $campoId = $fila['id_Campo'];
}

# labor
$consulta12 = sqlsrv_query($conn,("SELECT id_Grupo as grupo_id FROM Labor WHERE nombre = '$labor'" ));
while ($fila3 = sqlsrv_fetch_array($consulta12, SQLSRV_FETCH_ASSOC)) {
    $grupoId = $fila3['grupo_id'];
}


$consultaGrupo = sqlsrv_query($conn,("SELECT * FROM Grupo WHERE id_Grupo = '$grupoId'" ));
while ($fila1 = sqlsrv_fetch_array($consultaGrupo,SQLSRV_FETCH_ASSOC)) {
    $nombreGrupo=trim($fila1['nombre']);
}

$consultaMaquinaria = sqlsrv_query($conn,("SELECT * FROM maquinaria WHERE campo_id = '$campoId' OR campo_id = NULL" ));

$consultaImplemento = sqlsrv_query($conn,("SELECT * FROM implementos WHERE campo_id = '$campoId'" ));
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#terminar').click(function() {
                ...
                $.ajax({
                    url: 'actualizar_presupuesto.php?id=<?php echo $id ?>',
                    method: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        window.location='../orden-trabajo/orden-trabajo.php';
                    },
                    error: function(response) {
                        console.log(response);
                    }
